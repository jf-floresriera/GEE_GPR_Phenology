...
class LSPGenerationAlgorithm(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(QgsProcessingParameterBoolean(
            self.ADD_TO_QGIS,
            self.tr('Cargar resultado como capa raster en QGIS'),
            defaultValue=True
        ))
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        n_pix = n_rows * n_cols
        feedback.pushInfo(f'Dimensiones de referencia: {n_rows} x {n_cols} = {n_pix} píxeles')

        def _read_aligned_band1(fpath):
            with rasterio.open(fpath) as src:
                src_nodata = src.nodata if src.nodata is not None else nodata
                if (src.height == n_rows and src.width == n_cols and
        ...
        for date_str, fpath in sorted(file_dates.items()):
            ...
            try:
                ...
                feedback.pushWarning(f'Omitido {os.path.basename(fpath)}: {ex}')

        if len(arrays_list) < 6:
            raise QgsProcessingException(f'Se necesitan ≥6 imágenes válidas después de armonizar. Disponibles: {len(arrays_list)}')
        ...
        # The original Earth Engine regressions use the available valid observations
        # per pixel. For the local implementation, require at least 6 finite dates
        # instead of forcing every date to be valid.
        ...
        if add_to_qgis:
            ...
            queue_raster_layer(context, output_path, 'LSP_metrics', feedback, group_name=self.tr('GEEGPRPheno - LSP'), style_kind='lsp', variable='LSP_DOY')
