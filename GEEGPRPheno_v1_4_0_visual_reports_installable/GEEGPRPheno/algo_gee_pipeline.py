...
class GEEAutoPipelineAlgorithm(QgsProcessingAlgorithm):
    ...
    def _build_s2_mask(self, ee, img, mask_mode_i, band_names=None, feedback=None):
        ...
        if has_qa60:
            ...
            if feedback is not None:
                feedback.pushInfo('  aviso: QA60 no disponible; se aplica máscara basada en SCL.')

        if not has_scl:
            if feedback is not None:
                feedback.pushInfo('  aviso: SCL no disponible; se usa QA60 si existe, o sin máscara si no existe.')
            return qa_clear
    ...
    def _queue_outputs_to_qgis(self, context, raw_folder, pred_folder, gf_folder, lsp_output, veg_index, pred_dates, feedback):
        ...
        def _queue_folder(folder, suffix, prefix):
            files = []
            ...
            for fpath in files:
                ...
                queue_raster_layer(context, fpath, f'{prefix}_{base}', feedback)

        _queue_folder(raw_folder, '_S2.tif', 'S2_raw')
        _queue_folder(pred_folder, f'_{veg_index}.tif', f'GPR_pred_{veg_index}')
        _queue_folder(gf_folder, f'_{veg_index}.tif', f'GPR_gapfilled_{veg_index}')
    ...
    def _generate_pdf_reports(self, raw_folder, pred_folder, gf_folder, lsp_output, veg_index, out_folder, feedback):
        ...
        try:
            ...
            feedback.pushWarning(f'No se pudieron generar PDF: matplotlib no disponible ({ex})')
            return []
        ...
        try:
            ...
            feedback.pushWarning(f'No se pudo guardar CSV resumen: {ex}')

        pdf_paths = []
        summary_pdf = os.path.join(reports_dir, f'reporte_resumen_{veg_index}.pdf')
        with PdfPages(summary_pdf) as pdf:
            fig = plt.figure(figsize=(11.69, 8.27))
            fig.text(0.06, 0.92, f'Reporte GEE GPR Phenology — {veg_index}', fontsize=18, weight='bold')
            fig.text(0.06, 0.87, 'Contenido del reporte:', fontsize=12, weight='bold')
            fig.text(0.08, 0.83, '• Series temporales de estadisticos espaciales (prediccion y gapfilling).', fontsize=11)
            fig.text(0.08, 0.79, '• Mapas de la ultima fecha disponible para prediccion GPR y gapfilling.', fontsize=11)
            if lsp_output:
                fig.text(0.08, 0.75, '• Resumen de metricas LSP y atlas de bandas en PDF adicional.', fontsize=11)
            fig.text(0.06, 0.68, f'N. predicciones: {len(pred_stats)}', fontsize=11)
            fig.text(0.06, 0.64, f'N. gapfilled: {len(gf_stats)}', fontsize=11)
            fig.text(0.06, 0.60, f'Carpeta reportes: {reports_dir}', fontsize=10)
            fig.text(0.06, 0.56, f'CSV resumen: {csv_path}', fontsize=10)
            fig.text(0.06, 0.10, 'Nota: Los estadisticos se calculan con pixeles validos, excluyendo nodata.', fontsize=9)
            fig.tight_layout()
            pdf.savefig(fig)
            plt.close(fig)
            ...
            if pred_stats or gf_stats:
                ...
                if pred_stats:
                    ...
                    ax.plot(x, mean, marker='o', label='Prediccion GPR — media')
                    ax.fill_between(x, p25, p75, alpha=0.2, label='Prediccion GPR — IQR')
                if gf_stats:
                    ...
                    ax.plot(x2, mean2, marker='s', label='Gapfilled GPR — media')
                    ax.fill_between(x2, p25_2, p75_2, alpha=0.2, label='Gapfilled GPR — IQR')
                    labels = labels2 if not pred_stats else labels
                ax.set_title(f'Serie temporal de {veg_index}')
                ax.set_xlabel('Fecha')
            ...
            map_candidates = []
            if pred_files:
                map_candidates.append(('Ultima prediccion GPR', pred_files[-1]))
            if gf_files:
                map_candidates.append(('Ultimo gapfilling GPR', gf_files[-1]))
        ...
        if lsp_output and os.path.exists(lsp_output):
            ...
            with rasterio.open(lsp_output) as src, PdfPages(lsp_pdf) as pdf:
                ...
                fig = plt.figure(figsize=(11.69, 8.27))
                fig.text(0.06, 0.92, f'Atlas de metricas LSP — {veg_index}', fontsize=18, weight='bold')
                ypos = 0.84
                for i in range(min(n_bands, len(band_labels))):
                    fig.text(0.08, ypos, f'• Banda {i+1}: {band_labels[i]}', fontsize=11)
                    ypos -= 0.045
                fig.text(0.06, 0.10, 'Cada pagina siguiente muestra el mapa espacial y un histograma resumido.', fontsize=10)
                pdf.savefig(fig)
                plt.close(fig)
                ...
                for i in range(1, n_bands + 1):
                    ...
                    if vals.size:
                        ...
                        ax2.set_title(f'Histograma {label}')
                        ax2.set_xlabel(label)
                        ax2.set_ylabel('Frecuencia')
                    else:
                        ax2.text(0.5, 0.5, 'Sin datos validos', ha='center', va='center')
                        ax2.set_axis_off()
                    fig.tight_layout()
                    pdf.savefig(fig)
                    plt.close(fig)
