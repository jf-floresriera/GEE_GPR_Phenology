        ...
        if len(self.points) < 3:
            try:
                self.iface.messageBar().pushWarning(
                    "GEEGPRPheno", "Dibuja al menos 3 vértices y cierra con clic derecho."
                )
            except Exception:
                pass
            return
...
class GEEPanelDialog(QDialog):
    ...
    def _build_ui(self):
        ...
        ver = QLabel("GEEGPRPheno v1.4.0  —  UNAL Laboratorio 227")
    ...
    def _make_algo_tab(self, name, algo_id, icon_char, description, color):
        ...
        if algo_id == 'gee_auto_pipeline':
            aoi_btn = QPushButton("  Dibujar AOI en el mapa")
    ...
    def _start_draw_aoi(self):
        ...
        try:
            ...
            QMessageBox.critical(self, "GEEGPRPheno", f"No se pudo activar la herramienta AOI:\n{ex}")
    ...
    def _finish_draw_aoi(self, points):
        ...
        try:
            ...
            self.iface.messageBar().pushSuccess(
                "GEEGPRPheno", "AOI dibujada creada y activada. Selecciónala como AOI_LAYER en el pipeline."
            )
            try:
            ...
            QMessageBox.critical(self, "GEEGPRPheno", f"No se pudo crear la capa AOI:\n{ex}")
    ...
    def _make_info_tab(self):
        ...
        ver = QLabel("Version 1.4.0  |  QGIS 3.x  |  Python 3.x")
...
class GEEGPRPhenoPlugin:
    ...
    def initGui(self):
        ...
        act_aoi = QAction("Dibujar poligono AOI en el mapa", self.iface.mainWindow())
        ...
        act_install = QAction("Instalar/actualizar dependencias Python", self.iface.mainWindow())
        ...
        # Desde v1.4.0 el panel NO se abre automáticamente al iniciar QGIS.
    ...
    def _start_draw_aoi(self):
        ...
        try:
            ...
            QMessageBox.critical(self.iface.mainWindow(), "GEEGPRPheno", f"No se pudo activar la herramienta AOI:\n{ex}")
    ...
    def _finish_draw_aoi(self, points):
        ...
        try:
            ...
            QMessageBox.critical(self.iface.mainWindow(), "GEEGPRPheno", f"No se pudo crear la capa AOI:\n{ex}")
    ...
    def _install_dependencies(self):
        """Instala dependencias bajo demanda, nunca durante el arranque de QGIS."""
        try:
            from .installer import install_command, run_dependency_install
            cmd1, cmd2 = install_command()
            msg = (
                "El instalador ejecutará pip con el Python asociado a QGIS.\n\n"
                "Esto puede tardar varios minutos y requiere internet.\n\n"
                "Comandos:\n"
                f"{' '.join(cmd1)}\n"
                f"{' '.join(cmd2)}\n\n"
                "¿Deseas continuar?"
            ...
            if ret != QMessageBox.Yes:
                return
            ok, log_path, code = run_dependency_install()
            if ok:
                QMessageBox.information(
                    self.iface.mainWindow(),
                    ...
                    f"Log:\n{log_path}\n\n"
                    "Cierra y vuelve a abrir QGIS para asegurar que Python recargue los paquetes."
                )
            else:
                QMessageBox.warning(
                    self.iface.mainWindow(),
                    "Instalación incompleta",
                    "pip terminó con error.\n\n"
                    f"Código: {code}\nLog:\n{log_path}\n\n"
                    "Abre el log para ver la causa. En Windows, si falla, usa OSGeo4W Shell."
                )
        except Exception as ex:
            QMessageBox.critical(self.iface.mainWindow(), "GEEGPRPheno", f"No se pudo ejecutar pip:\n{ex}")
