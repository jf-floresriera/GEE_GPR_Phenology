...
def run_dependency_install(log_path=None):
    """
    Public repository safety mode: do not execute pip or shell commands from QGIS.

    The official QGIS Plugin Repository performs automated security scans. For that
    reason, dependency installation is documented through manual instructions rather
    than executed by the plugin.
    """
    if log_path is None:
        log_path = os.path.join(Path.home(), "GEEGPRPheno_dependency_install_README.txt")
    try:
        with open(log_path, "w", encoding="utf-8") as log:
            log.write("GEEGPRPheno dependency installation instructions\n\n")
            log.write(manual_install_instructions())
            log.write("\n")
    except Exception:
        pass
    return False, log_path, -1
